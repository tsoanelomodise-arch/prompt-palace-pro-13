...
import { ArrowLeft, ChevronRight, FileText, Plus, BookOpen } from "lucide-react";
      ...
      {nodes.map((n) => {
        ...
        return (
          <li key={n.id}>
            ...
            >
              {kids.length > 0 ? <ChevronRight className="h-3 w-3 shrink-0" /> : <FileText className="h-3 w-3 shrink-0" />}
              <span className="truncate">{n.title}</span>
              {n.status === "draft" && (
                <span className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground ml-auto">
                  draft
                </span>
              )}
            </Link>
